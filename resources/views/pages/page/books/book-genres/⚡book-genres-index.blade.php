...
new class extends Component
{
    ...
    public $file;
    public $titlePage = 'Genero de libros';
    public $subtitlePage = 'Listado con generos de libros';

    //////////////////////////////////////////////////////////////////// CONSULTA DE LISTADO Y ELIMINAR ITEM
    // consulta de item
    public function queryBookGenres(){
        return BookGenre::where('user_id', Auth::id())
            ->where(function ($query) {
                $query->where('name', 'like', "%{$this->search}%")
                      ->orWhere('slug', 'like', "%{$this->search}%")
                      ->orWhere('name_general', 'like', "%{$this->search}%")
                      ->orWhere('slug_general', 'like', "%{$this->search}%");
            })
    ...
    public function deleteItem($codigo){
        $item = BookGenre::where('user_id', Auth::id())->where('uuid', $codigo)->first();
        $item->delete();
    }
...
<div>
    ...
    <div>
        <div container class="mb-1 space-y-1">
            <flux:heading size="xl" level="1">
                <a href="{{ route('book-genres.create') }}"><flux:button size="xs" variant="ghost" icon="plus"></flux:button></a>
                {{ $this->titlePage }}
            </flux:heading>
    ...
    {{-- listado de sujetos --}}
    <div class="space-y-2">
        @foreach ($this->queryBookGenres() as $item)
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center justify-center">
                        <a href="{{ route('book-genres.edit', ['bookGenreUuid' => $item->uuid]) }}"><flux:button size="xs" variant="ghost" icon="pencil-square"></flux:button></a>
    ...
    <div class="flex justify-between items-center gap-1">
        <flux:button icon="cloud-arrow-down" class="text-xs text-center" wire:click="export('book_genres')">Exp.</flux:button>
        <div class="flex gap-3">
            <flux:button icon="cloud-arrow-up" class="text-xs text-center" wire:click="import('book_genres')">Imp.</flux:button>
