...
new class extends Component
{
    ...
    // consulta de item
    public function queryBltags(){
        return Bltag::where('user_id', Auth::id())
    ...
    public function deleteItem($uuid){
        $item = Bltag::where('user_id', Auth::id())->where('uuid', $uuid)->first();
        $item->delete();
    }
...
<div>
    ...
    <div>
        <div container class="mb-1 space-y-1">
            <flux:heading size="xl" level="1">
                <a href="{{ route('bltags.create') }}"><flux:button size="xs" variant="ghost" icon="plus"></flux:button></a>
                {{ $this->titlePage }}
            </flux:heading>
    ...
    {{-- listado de sagas --}}
    <div class="space-y-2">
        @foreach ($this->queryBltags() as $item)
            <flux:badge rounded color="green">
                <a href="{{ route('blogs.index', ['tag' => $item->uuid]) }}">
                    <span class="text-xs">#{{ $item->name }}</span>
                </a>
                <div class="flex items-center justify-center ml-5">
                        <a href="{{ route('bltags.edit', ['bltagUuid' => $item->uuid]) }}"><flux:button size="xs" variant="ghost" icon="pencil-square"></flux:button></a>
    ...
    <div class="flex justify-between items-center gap-1">
        <flux:button icon="cloud-arrow-down" class="text-xs text-center" wire:click="export('bltag')">Exp.</flux:button>
        <div class="flex gap-3">
            <flux:button icon="cloud-arrow-up" class="text-xs text-center" wire:click="import('bltag')">Imp.</flux:button>
