...
new class extends Component
{
    ...
    public function mount(){
        $series = $this->serieQuery()->get();

        $this->series = collect($series)
            ->groupBy(function ($serie) {
                return $serie->views_max_end_view
                    ? \Carbon\Carbon::parse($serie->views_max_end_view)->format('Y')
                    : 'Sin fecha';
            })
            ->sortKeysDesc();
