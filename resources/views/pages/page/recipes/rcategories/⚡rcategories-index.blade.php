...
new class extends Component
{
    ...
    // consulta de item
    public function queryRcategories(){
        return Rcategory::where('user_id', Auth::id())
    ...
    public function deleteItem($codigo){
        $item = Rcategory::where('user_id', Auth::id())->where('uuid', $codigo)->first();
        $item->delete();
    }
...
<div>
    ...
    <div>
        <div container class="mb-1 space-y-1">
            <flux:heading size="xl" level="1">
                <a href="{{ route('rcategories.create') }}"><flux:button size="xs" variant="ghost" icon="plus"></flux:button></a>
                {{ $this->titlePage }}
            </flux:heading>
    ...
    {{-- listado de sagas --}}
    <div class="space-y-2">
        @foreach ($this->queryRcategories() as $item)
            <flux:badge rounded color="green">
                <a href="{{ route('recipes.index', ['cat' => $item->uuid]) }}">
                    <span class="text-xs">#{{ $item->name }}</span>
                </a>
                <div class="flex items-center justify-center ml-5">
                        <a href="{{ route('rcategories.edit', ['rcategoryUuid' => $item->uuid]) }}"><flux:button size="xs" variant="ghost" icon="pencil-square"></flux:button></a>
    ...
    {{-- paginacion --}}
    <div class="mt-3">
        {{ $this->queryRcategories()->links() }}
    </div>

    {{-- exportacion e importacion de excel --}}
    <flux:separator class="mb-2 mt-10" variant="subtle" />
    <div class="flex justify-between items-center gap-1">
        <flux:button icon="cloud-arrow-down" class="text-xs text-center" wire:click="export('rcategories')">Exp.</flux:button>
        <div class="flex gap-3">
            <flux:button icon="cloud-arrow-up" class="text-xs text-center" wire:click="import('rcategories')">Imp.</flux:button>
